...
export default function AccountScreen({ isRTL, toast, currentUser, changePassword }: Props) {
  ...
  function submit(e: React.FormEvent) {
    e.preventDefault();
    setError('');
    if (current !== currentUser.password) { setError(isRTL ? 'كلمة المرور الحالية غير صحيحة' : 'Current password is incorrect'); return; }
    if (next.length < 4) { setError(isRTL ? 'كلمة المرور قصيرة جداً' : 'New password is too short'); return; }
        ...
        <div className="max-w-md mx-auto space-y-5">
          <div className="bg-white dark:bg-[#1a1d26] border border-slate-200 dark:border-slate-700/50 rounded-2xl p-5 flex items-center gap-4">
            ...
            <div>
              ...
              <div className="text-slate-400 text-xs font-mono">@{currentUser.username}</div>
              <span className={`inline-block mt-1 text-xs px-2.5 py-0.5 rounded-full font-medium ${currentUser.role === 'admin' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/20 dark:text-blue-400' : 'bg-slate-100 text-slate-600 dark:bg-[#252a36] dark:text-slate-300'}`}>
                {currentUser.role === 'admin' ? (isRTL ? 'مدير' : 'Admin') : (isRTL ? 'كاشير' : 'Cashier')}
              </span>
            </div>
          </div>

          <form onSubmit={submit} className="bg-white dark:bg-[#1a1d26] border border-slate-200 dark:border-slate-700/50 rounded-2xl p-5 space-y-4">
            <div className="font-semibold text-slate-900 dark:text-slate-100 text-sm">{isRTL ? 'تغيير كلمة المرور' : 'Change Password'}</div>
            {([
              [isRTL ? 'كلمة المرور الحالية' : 'Current password', current, setCurrent],
              [isRTL ? 'كلمة المرور الجديدة' : 'New password', next, setNext],
              [isRTL ? 'تأكيد كلمة المرور' : 'Confirm new password', confirm, setConfirm],
            ...
            {error && <div className="text-red-600 dark:text-red-400 text-xs bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/50 rounded-lg px-3 py-2">{error}</div>}
            <button type="submit" className="w-full py-2.5 bg-blue-600 hover:bg-blue-500 text-white font-semibold rounded-xl text-sm transition-colors">
