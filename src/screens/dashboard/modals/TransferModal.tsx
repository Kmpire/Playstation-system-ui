...
import type { GameConsole, ConsoleType } from '../../../types';
...
export default function TransferModal({
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      isRTL={isRTL}
      title={`${isRTL ? 'نقل الجلسة من:' : 'Transfer Session from:'} ${fromCon.name}`}
      subtitle={isRTL ? 'اختر الجهاز المتاح لنقل الجلسة إليه' : 'Select an available destination console'}
      ...
      <div className="space-y-3">
        {targets.length === 0 ? (
          <div className="text-center py-8 text-slate-400 text-sm">
            {isRTL ? 'لا توجد أجهزة متاحة شاغرة حالياً للنقل.' : 'No available consoles at the moment.'}
          </div>
        ) : (
