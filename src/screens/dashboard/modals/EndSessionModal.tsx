...
import { Square, CheckCircle2, Clock, User, Users, Coffee, AlertCircle } from 'lucide-react';
...
export default function EndSessionModal({
  ...
  return (
    <Modal
      ...
      subtitle={isRTL ? 'ملخص الحساب والطلبات' : 'Session summary and cash payment'}
        ...
        {session.tab.length > 0 && (
          <div className="space-y-1.5">
            <div className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center justify-between">
              <span>{isRTL ? 'طلبات المشروبات والمأكولات' : 'Snack Tab Items'}</span>
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-slate-700 dark:text-slate-300 font-medium">
                      {isRTL && item.nameAr ? item.nameAr : item.name} × {item.qty}
                    </span>
                  </div>
        ...
        {isEarly ? (
          <div className="space-y-2 pt-2 border-t border-slate-200 dark:border-slate-800">
            ...
            >
              <div>
                <div className="text-sm font-bold text-slate-900 dark:text-white">
                  {isRTL ? 'تحصيل المبلغ المحجوز كاملاً' : 'Charge Full Booked Amount'}
                </div>
            ...
            >
              <div>
                <div className="text-sm font-bold text-slate-900 dark:text-white">
                  {isRTL ? 'محاسبة على الوقت الفعلي فقط' : 'Charge for Actual Time Played'}
                </div>
                <div className="text-xs text-slate-500 dark:text-slate-400">
                  {isRTL ? 'خصم الوقت المتبقي للزبون' : 'Discount remaining minutes'}
                </div>
              </div>
