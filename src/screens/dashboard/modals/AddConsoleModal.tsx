...
const TYPE_CONFIG: Record<ConsoleType, { label: string; icon: React.ReactNode }> = {
  PS4: { label: 'PlayStation 4', icon: <Gamepad2 className="w-4 h-4 text-sky-400" /> },
  PS5: { label: 'PlayStation 5', icon: <Tv className="w-4 h-4 text-[#0070d1]" /> },
  Xbox: { label: 'Xbox Series', icon: <Gamepad2 className="w-4 h-4 text-emerald-400" /> },
  VIP: { label: 'VIP Room', icon: <Crown className="w-4 h-4 text-amber-400" /> },
};
...
export default function AddConsoleModal({
  ...
  return (
    <Modal
      ...
      subtitle={isRTL ? 'حدد اسم ونوع الجهاز لتسجيله في الصالة' : 'Enter name and model of the console'}
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input
            ...
            placeholder={isRTL ? 'مثال: PS5 — 05 أو غرفة VIP 2' : 'e.g. PS5 — 05'}
            autoFocus
