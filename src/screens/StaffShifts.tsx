  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-[#0f111a] overflow-hidden">
      ...
      <div className="bg-white dark:bg-[#1a1d26] border-b border-slate-200 dark:border-slate-700/50 px-6 py-4 shrink-0">
        ...
        <p className="text-slate-500 dark:text-slate-500 text-sm">{isRTL ? 'إدارة الموظفين والورديات وسجل المراجعة' : 'Staff management, shift handover, and audit trail'}</p>
      </div>
      ...
      <div className="bg-white dark:bg-[#1a1d26] border-b border-slate-200 dark:border-slate-700/50 px-6 flex gap-0 shrink-0">
        {TABS.map(tab => (
          <button key={tab.id} onClick={() => setSubTab(tab.id)}
            className={`px-5 py-3 text-sm font-medium border-b-2 transition-colors -mb-px ${subTab === tab.id ? 'border-blue-600 text-blue-600 dark:text-blue-400' : 'border-transparent text-slate-500 dark:text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'}`}>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {subTab === 'staff' && (
          <div className="max-w-2xl space-y-4">
            {STAFF.map(s => (
              <div key={s.id} className="bg-white dark:bg-[#1a1d26] border border-slate-200 dark:border-slate-700/50 rounded-2xl p-5 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-semibold text-slate-900 dark:text-slate-100">{isRTL ? s.nameAr : s.name}</div>
                  <div className="text-slate-400 text-xs mt-0.5 font-mono">@{s.username}</div>
                </div>
                <div className="text-end">
                  <span className={`inline-block text-xs px-2.5 py-1 rounded-full font-medium ${s.role === 'Admin' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/20 dark:text-blue-400' : 'bg-slate-100 text-slate-600 dark:bg-[#252a36] dark:text-slate-300'}`}>
                    {s.role}
                  </span>
                  <div className="text-slate-400 text-[11px] mt-1">{isRTL ? `منذ ${s.since}` : `Since ${s.since}`}</div>
                </div>
              </div>
            ))}
        ...
        {subTab === 'shift' && (
          <div className="max-w-lg space-y-5">
            {submitted && (
              <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800/50 rounded-xl p-4 text-green-700 dark:text-green-400 text-sm flex items-center gap-2">
                ✅ {isRTL ? 'تم إرسال تقرير الوردية بنجاح.' : 'Shift report submitted successfully.'}
              </div>
            )}

            <div className="bg-white dark:bg-[#1a1d26] border border-slate-200 dark:border-slate-700/50 rounded-2xl p-5 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                {/* Expected */}
                <div className="bg-slate-50 dark:bg-[#252a36] rounded-xl p-4">
                  <div className="text-xs text-slate-500 dark:text-slate-500 uppercase tracking-wider mb-2">{isRTL ? 'النقد المتوقع (آلي)' : 'Expected Cash (auto)'}</div>
                  <div className="text-2xl font-mono font-bold text-slate-900 dark:text-slate-100">{money(expectedCash, isRTL)}</div>
                </div>

                {/* Counted */}
                <div className="bg-white dark:bg-[#1a1d26] border border-slate-200 dark:border-slate-700/50 rounded-xl p-4">
                  <div className="text-xs text-slate-500 dark:text-slate-500 uppercase tracking-wider mb-2">{isRTL ? 'النقد المعدود' : 'Counted Cash'}</div>
