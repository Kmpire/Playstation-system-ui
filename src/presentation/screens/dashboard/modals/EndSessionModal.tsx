  ...
  return (
    <Modal
      ...
      subtitle={
        isBreak
          ? isRTL
            ? "ملخص طلبات ومشروبات الاستراحة"
            : "Break lounge orders summary and cash payment"
          : isRTL
            ? "ملخص الحساب والطلبات"
            : "Session summary and cash payment"
      }
      icon={
        isBreak ? (
        ...
        {isBreak ? (
          <div className="rounded-2xl bg-emerald-500/10 border border-emerald-500/20 p-3.5 flex items-center justify-between">
            <div className="flex items-center gap-2 text-emerald-600 dark:text-emerald-400 text-sm font-semibold">
              ...
              <span>{isRTL ? "جلسة استراحة (بدون وقت)" : "Lounge Tab (No hourly fee)"}</span>
            </div>
            <span className="text-xs text-slate-500">
              {isRTL ? `${session.tab.length} طلبات` : `${session.tab.length} items`}
            </span>
          </div>
        ) : (
          <>
