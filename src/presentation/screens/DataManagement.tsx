    ...
    <div className="h-full overflow-y-auto bg-slate-50 dark:bg-[#0f111a]">
      <div className="bg-white dark:bg-[#1a1d26] border-b border-slate-200 dark:border-slate-700/50 px-4 sm:px-6 py-3.5 sm:py-4">
        ...
        <p className="text-slate-500 dark:text-slate-500 text-xs sm:text-sm">
          {isRTL
            ? "تصدير واستيراد نسخة كاملة من قاعدة البيانات"
            : "Export and import a full snapshot of the database"}
        </p>
      </div>
      ...
      <div className="p-4 sm:p-6 pb-24 sm:pb-8">
        <div className="max-w-2xl mx-auto space-y-4 sm:space-y-5">
            ...
            <div className="flex items-start gap-3.5 sm:gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-slate-500 dark:text-slate-500 text-xs sm:text-sm mt-1 mb-3 sm:mb-4 leading-relaxed">
                  {isRTL
                    ? "حفظ لقطة كاملة لجميع البيانات (الأجهزة، القائمة، الأسعار، الصيانة، الورديات) كملف JSON."
                    : "Save a full snapshot of all data (consoles, menu, pricing, maintenance, shifts) to a JSON file."}
                </p>
                <button
                  onClick={exportData}
                  className="w-full sm:w-auto px-5 py-2.5 bg-blue-600 hover:bg-blue-500 text-white font-semibold rounded-xl text-xs sm:text-sm transition-colors text-center active:scale-98"
                >
          ...
          <div className="bg-white dark:bg-[#1a1d26] border border-slate-200 dark:border-slate-700/50 rounded-2xl p-4 sm:p-6">
            <div className="flex items-start gap-3.5 sm:gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-slate-500 dark:text-slate-500 text-xs sm:text-sm mt-1 mb-3 sm:mb-4 leading-relaxed">
                  {isRTL
                    ? "استعادة قاعدة البيانات من آخر نسخة محفوظة (مثلاً بعد إعادة تثبيت التطبيق أو النقل لجهاز آخر)."
                    : "Restore the database from a previously saved snapshot (e.g. after reinstalling or moving to another device)."}
                </p>
                <input
                  ref={fileRef}
                  type="file"
                ...
                <button
                  ...
                  className="w-full sm:w-auto px-5 py-2.5 border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800/50 font-semibold rounded-xl text-xs sm:text-sm transition-colors text-center active:scale-98"
                >
          ...
          <div className="text-xs text-slate-400 text-center">
            {lastBackup
              ? isRTL
                ? `آخر نسخة احتياطية: ${new Date(lastBackup).toLocaleString()}`
