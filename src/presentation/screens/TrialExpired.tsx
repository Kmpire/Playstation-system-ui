...
export default function TrialExpired({ isRTL, onActivate }: Props) {
  ...
  return (
    <div
      className="min-h-screen bg-[#0b0d12] flex items-center justify-center p-6"
      dir={isRTL ? "rtl" : "ltr"}
    >
      <div className="w-full max-w-md text-center">
        <div className="text-6xl mb-5">🔒</div>
        <h1 className="text-white text-2xl font-bold font-gaming mb-2">
          {isRTL ? "انتهت الفترة التجريبية" : "Trial Expired"}
        </h1>
        ...
        <form
          onSubmit={submit}
          className="bg-[#141720] border border-[#252a36] rounded-2xl p-6 space-y-4"
        >
          <input
            value={code}
            onChange={(e) => {
          ...
          <button
            type="submit"
            className="w-full bg-cyan-500 hover:bg-cyan-400 text-black font-bold py-3 rounded-xl text-sm font-gaming transition-colors"
          >
            {isRTL ? "تفعيل" : "Activate"}
          </button>
          <div className="text-[11px] text-slate-600">
            {isRTL ? "تجريبي:" : "Demo code:"} PSCAFE-PRO-2026
          </div>
        </form>
      </div>
    </div>
