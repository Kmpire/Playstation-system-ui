...
import type { AuditEntry, UserRole, ShiftReport } from "@/domain"
            ...
            <div className="bg-white dark:bg-[#1a1d26] border border-slate-200 dark:border-slate-700/50 rounded-2xl p-4 sm:p-5 space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                ...
                <div className="bg-slate-50 dark:bg-[#252a36] rounded-xl p-3.5 sm:p-4">
                  <div className="text-[11px] sm:text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">
                    {isRTL ? "النقد المتوقع (آلي)" : "Expected Cash (auto)"}
                  </div>
                  <div className="text-xl sm:text-2xl font-mono font-bold text-slate-900 dark:text-slate-100">
                    {money(expectedCash, isRTL)}
                  </div>
                </div>
                ...
                <div className="bg-white dark:bg-[#1a1d26] border border-slate-200 dark:border-slate-700/50 rounded-xl p-3.5 sm:p-4">
                  <div className="text-[11px] sm:text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">
                    {isRTL ? "النقد المعدود" : "Counted Cash"}
                  </div>
                  <div className="relative">
                    <span className="absolute start-2 top-1/2 -translate-y-1/2 text-slate-400 font-mono text-xs">
                      {isRTL ? "ج.م" : "EGP"}
                    </span>
                    <input
                      type="number"
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <div className="ms-auto text-slate-400 text-xs">
                {filteredLog.length} {isRTL ? "سجل" : "entries"}
              </div>
            </div>
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-slate-50 dark:divide-slate-700/30">
                  {filteredLog.map((entry) => (
                    ...
                    >
                      <td className="px-4 py-3 text-xs font-mono text-slate-500 dark:text-slate-500 whitespace-nowrap">
                        {entry.timestamp}
                      </td>
                      ...
                      <td className="px-4 py-3 text-xs text-slate-700 dark:text-slate-300 font-medium whitespace-nowrap">
                        {entry.actionType}
                      </td>
